...
var pluginHelper = {
  ...
  cannotLoad: function (plugin) {
    ...
    if (_.has(plugin, 'dependencies')) var error = false;
    ...
    _.each(plugin.dependencies, function (dep) {
      try {
        ...
        if (!e.message.startsWith('Cannot find module')) return util.die(e);
  ...
  load: function (plugin, next) {
    ...
    if (!plugin.config || !plugin.config.enabled) return next();

    console.log(plugin.modes, gekkoMode);

    if (plugin.modes.includes(gekkoMode)) {
      log.warn(
        'The plugin',
        plugin.name,
    ...
    if (plugin.path) var Constructor = require(pluginDir + plugin.path(config));
    else var Constructor = require(pluginDir + plugin.slug);

    if (plugin.async) {
      inherits(Constructor, Emitter);
      var instance = new Constructor(
        util.defer(function (err) {
          next(err, instance);
        }),
        plugin
      );
